...
static inline cstr_view cstr_view_from(const char* str) {
    return (cstr_view){str, str ? strlen(str) : 0};
}
...
static inline size_t cstr_view_find_substr(cstr_view str, cstr_view substr) {
    ...
    if (substr.length > str.length) return CSTR_NPOS;
...
// === View access ===
static inline char cstr_view_first(cstr_view str) {
    return (str.length == 0 || str.data == NULL) ? '\0' : str.data[0];
}
